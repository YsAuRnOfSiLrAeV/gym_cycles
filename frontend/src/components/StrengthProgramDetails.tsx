...
export default function HypertrophyProgramDetails() {
  ...
  const onGenerate = async () => {
    setLoading(true);
    try {
      const response = await fetch('/api/strength/cycle', {
        method: 'POST',
  ...
  function exportPdf(program: TrainingWeek[]) {
    ...
    doc.setFontSize(18);
    doc.text('Strength Program', margin + 80, 65);
  ...
  async function exportExcel(program: TrainingWeek[]) {
    ...
    saveAs(new Blob([buffer]), 'strength_program.xlsx');
  }
    ...
    <div className="flex-1 bg-[#eeeeee] border-t-12 border-[#D0D0D0]">
      ...
      <div className="flex flex-col lg:flex-row lg:gap-3">
        <div className="flex-1 pr-0 lg:pr-8">
          <h1 className="text-3xl font-poppins font-semibold mb-6">Power cycle</h1>
          <p className="mb-4">
            Strength cycle for a powerlifter that is suitable for athletes at the II-KMS level, since it was tested on athletes of that level and has proven to be effective. The program uses linear periodization with a gradual increase in training intensity. The cycle is designed for people who have a high level of exercise execution, so some adjustments are permitted during the implementation of this mesocycle. The athlete must have good flexibility, correct joint angles, and overall technique must necessarily be at a high level.
          </p>
        ...
        <div className="w-full lg:w-80 mt-6 lg:mt-0">
          ...
          <div className="space-y-4 mb-6">
            ...
            <div>
              <label className="block mb-1">Your dead lift PR:</label>
              <input
                type="number"
                value={deadliftPr}
